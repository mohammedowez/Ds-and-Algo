...
public:
    ...
    bool DFS(vector<vector<char>>& board, string word, int i, int j, int n) {
        ...
        bool    down = DFS(board, word, i + 1, j, n + 1) ;  //down
        bool    right= DFS(board, word, i, j + 1, n + 1) ;  //right
        bool    up=     DFS(board, word, i - 1, j, n + 1) ;  //up
        bool    left =   DFS(board, word, i, j - 1, n + 1);  //left
